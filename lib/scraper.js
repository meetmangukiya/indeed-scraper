...
module.exports.query = function(queryObject, options = {}){
  ...
  return q.getJobs().then((jobs) =>
  {
    if (options.getDescription) {
      return getAllDescriptions(jobs);
    }
...
function Query(qo) {
  ...
  this.requestOptions = qo.requestOptions || {};
  ...
  return new Promise((resolve, reject) => {
    ...
    function getSomeJobs(self, jobs) {
      request(Object.assign({
        url: self.url()
      }, self.requestOptions), (error, response, body) => {
...
const getDetail = (requestOptions) => {
  return new Promise((resolve, reject) => {
    request(requestOptions, (error, response, body) => {
      ...
      // Remove description meta info
      $('.jobsearch-JobMetadataHeader').remove();

      // Remove all attributes inside description. NB Can't use arrow function here.
      $('.jobsearch-JobComponent-description *').each(function() {
        this.attribs = {};
      });

      resolve({
        description: ($('.jobsearch-JobComponent-description').html() || '').trim(),
        expired: !!$('.jobsearch-JobInfoHeader-expiredHeader').html()
      });
    });
  })
}

const getAllDetails = (jobs, requestOptions) => {
  return Promise.all(jobs.map(
    job => {
      return new Promise(resolve => {
        getDetail(Object.assign({url: job.url}, requestOptions)).then((detail) => {
          resolve(Object.assign(detail, job))
        })
      })
    }
  ));
...
module.exports.query = function(queryObject, options = {}){
  ...
  return q.getJobs().then((jobs) =>
  {
    if (options.getDetails) {
      return getAllDetails(jobs, queryObject.requestOptions || {});
    }
