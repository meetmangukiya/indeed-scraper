...
function Query(qo){
  // query variables
  this.baseUrl = qo.baseUrl || 'https://www.indeed.com';
  this.query = qo.query || '';
  this.city = qo.city || '';
  ...
  return new Promise((resolve, reject) => {
    ...
    function getSomeJobs(self, jobs) {
      request(self.url(), (error, response, body) => {
        const parsed = parseJobList(body, baseUrl);
...
function parseJobList(body, baseUrl){
  ...
  const jobObjects = filtered.map((i, e) => {
    ...
    const url = baseUrl + job.find('.jobtitle').children('a').attr('href');
