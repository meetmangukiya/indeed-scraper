...
const getDescription = (requestOptions) => {
  return new Promise((resolve, reject) => {
    request(requestOptions, (error, response, body) => {
      ...
      resolve(($('.jobsearch-JobComponent-description').html() || '').trim());
    });
  })
}

const getAllDescriptions = (jobs, requestOptions) => {
  return Promise.all(jobs.map(
    job => {
      return new Promise(resolve => {
        getDescription(Object.assign({url: job.url}, requestOptions)).then((description) => {
...
module.exports.query = function(queryObject, options = {}){
  ...
  {
    if (options.getDescription) {
      return getAllDescriptions(jobs, queryObject.requestOptions || {});
    }
